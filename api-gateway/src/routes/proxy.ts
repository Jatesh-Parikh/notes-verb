...
function createServiceProxy(
    ...
    const options = {
        ...
        onProxyReq: (proxyReq: any, req: any) => {
            ...
            console.log(`Proxying request: ${req.method} ${req.originalUrl} to ${targetUrl}`);
...
router.use(
    ...
    createServiceProxy(servicesConfig.auth.url, {
        "^/api/auth": "/auth", // Rewrite path to match the service endpoint
    }
));
...
router.use(
    ...
    createServiceProxy(servicesConfig.users.url, {
        "^/api/users": "/users", // Rewrite path to match the service endpoint
    }
));
...
router.use(
    ...
    createServiceProxy(servicesConfig.notes.url, {
        "^/api/notes": "/notes", // Rewrite path to match the service endpoint
    }
));
...
router.use(
    ...
    createServiceProxy(servicesConfig.tags.url, {
        "^/api/tags": "/tags", // Rewrite path to match the service endpoint
    }
));
